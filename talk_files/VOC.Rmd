...
X <- X[,!apply(X,2,anyNA)]
X[,apply(X,2,is.numeric)] <- scale(X[,apply(X,2,is.numeric)])
```
...
cnord1 <- gllvm(Ya, X = X, studyDesign = X, num.lv.c = 2, 
               ...
               Ab.struct = "diagonal", optim.method = "L-BFGS-B")
gllvm::ordiplot(cnord1, symbols=TRUE, pch=X$island, s.colors = as.character(factor(X$island,labels  = c("blue", "green", "black"))), cex.env = 1.5, main = NA, xlim = c(-3,1))
```
